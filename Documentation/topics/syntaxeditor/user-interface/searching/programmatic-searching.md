...
The "view" search model is accessed via the [IEditorView](xref:ActiproSoftware.UI.WinForms.Controls.SyntaxEditor.IEditorView).[Searcher](xref:ActiproSoftware.UI.WinForms.Controls.SyntaxEditor.IEditorView.Searcher) property.  This property returns an [IEditorViewSearcher](xref:ActiproSoftware.UI.WinForms.Controls.SyntaxEditor.IEditorViewSearcher) instance, which has find/replace functionality built-in.
...
This view search model options interface adds a [Scope](xref:ActiproSoftware.UI.WinForms.Controls.SyntaxEditor.IEditorSearchOptions.Scope) property, which is an enumeration value of type [EditorSearchScope](xref:ActiproSoftware.UI.WinForms.Controls.SyntaxEditor.EditorSearchScope).  It specifies whether the search is performed within the entire document or in the current selection only.

The [EditorSearchOptions](xref:ActiproSoftware.UI.WinForms.Controls.SyntaxEditor.Implementation.EditorSearchOptions) is an implementation of the [IEditorSearchOptions](xref:ActiproSoftware.UI.WinForms.Controls.SyntaxEditor.IEditorSearchOptions) interface and can be used in your code.
