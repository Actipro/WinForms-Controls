...
In addition, customers with active WinForms Studio subscriptions will receive any new products added to the suite for free while the subscription is active.
...
This licensing option is the classic style of implementing licensing in .NET Framework applications, and is what Actipro exclusively used prior to v2021.1.  Note that .NET Framework applications can either use this licensing option or the `RegisterLicense` call licensing option described above.
...
1. If you have licensed the product, configure the "licenses.licx" file in the projects that reference the product.
    ...
    - Ensure that the "licenses.licx" has a correct entry for Actipro's WinForms control products.  The valid entry is listed below in this topic.  Ensure the version numbers in the entry always match the deployed assembly version, especially after upgrading to a new version or build.
...
The contents of a "licenses.licx" file are pretty simple.  It needs a single line that supplies license information to all Actipro WinForms control products for which you are licensed.  Make sure that the version numbers, including build, match exactly with the version number of the control that is installed.

This single line (update the version to match the one you use) should be added to the "licenses.licx" file in any project that uses Actipro WinForms control or SyntaxEditor add-on products:

```
ActiproSoftware.Products.ActiproLicenseToken, ActiproSoftware.Shared.WinForms, Version=21.1.0.0, Culture=neutral, PublicKeyToken=c27e062d3c1a4763
```
