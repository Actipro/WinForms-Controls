...
static class Program {
	...
	static void Main() {
		...
		#endif
		_logger = LoggerFactory.DefaultInstance.CreateLogger(typeof(Program));
	...
	private static void InitializeSyntaxEditor() {
		...
		appDataPath = Path.Combine(
			Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
			"Actipro Software",
			"WinForms SampleBrowser Python Package Repository"
		);
	...
	/// <summary>
	/// Indicates if the current .NET run-time automatically handles the scaling of most control fonts.
	/// </summary>
	/// <value>
	...
	/// <seealso cref="ActiproSoftware.UI.WinForms.Drawing.DpiHelper.RescaleFont(Font, SizeF, SizeF)"/>
	...
	/// <summary>
	/// Indicates if the current .NET run-time automatically handles the scaling of most control sizes.
	/// </summary>
	/// <value>
	...
	/// </value>
	/// <remarks>For use with per-monitor DPI awareness.</remarks>
	/// <seealso cref="ActiproSoftware.UI.WinForms.Drawing.DpiHelper.RescaleSize(Size, int, int)"/>
	...
	public static void LaunchExternalBrowser(string url) {
		if (string.IsNullOrWhiteSpace(url))
			return;
		_logger?.LogDebug("Launch external browser :: {0}", url);
	...
	public static Process? ShellExecute(string fileName) {
		_logger?.LogDebug("ShellExecute :: {0}", fileName);
