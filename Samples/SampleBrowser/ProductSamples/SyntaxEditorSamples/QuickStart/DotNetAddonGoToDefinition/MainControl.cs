...
public partial class MainControl : UserControl, IProductSample {
	...
	public MainControl() {
		...
		selectDefinitionCheckBox.Checked = _goToDefinitionService.IsWordSelectedOnNavigation;
	...
	private void OnGoToDefinitionButtonClick(object sender, EventArgs e) {
		...
		if ((resultSet is not null) && (resultSet.Results.Count > 0)) {
	...
	private void OnSelectDefinitionCheckBoxCheckedChanged(object sender, EventArgs e) {
		if (_goToDefinitionService is not null)
			_goToDefinitionService.IsWordSelectedOnNavigation = selectDefinitionCheckBox.Checked;
	}
