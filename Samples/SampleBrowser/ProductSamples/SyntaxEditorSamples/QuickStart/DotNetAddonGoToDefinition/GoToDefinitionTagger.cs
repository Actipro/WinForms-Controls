...
public class GoToDefinitionTagger : TaggerBase<IClassificationTag> {
	...
	static GoToDefinitionTagger() {
		...
		var style = new HighlightingStyle(foreground: Color.Blue) { UnderlineKind = LineKind.Solid };
		AmbientHighlightingStyleRegistry.Instance.Register(_goToDefinitionClassificationType, style);
	}
	...
	public GoToDefinitionTagger(IEditorView view)
		: base(nameof(GoToDefinitionTagger), [new Ordering(TaggerKeys.Token, OrderPlacement.Before)], view.SyntaxEditor.Document) {

		_view = view ?? throw new ArgumentNullException(nameof(view));

		// Ensure a classification type is registered to apply styles to tags
	...
	private void ApplyCustomCursor() {
		if (!_isCustomCursorActive) {
			_view.SyntaxEditor.OverrideCursor = Cursors.Hand;
	...
	private void ClearCustomCursor() {
		if (_isCustomCursorActive) {
			_view.SyntaxEditor.OverrideCursor = null;
	...
	private static bool DoesKeyImpactTagging(Keys key) {
	...
	public void NotifyKeyDown(KeyEventArgs e) {
		if (DoesKeyImpactTagging(e.KeyData) && _lastMouseLocation.HasValue) {
			// The state of the modifier key has changed, so tagging should be updated
			UpdateTaggedRange(_lastMouseLocation.Value);
		}
	}
	...
	public void NotifyPointerMoved(InputPointerEventArgs e) {
		...
		_lastMouseLocation = e.GetPosition(_view.SyntaxEditor);
