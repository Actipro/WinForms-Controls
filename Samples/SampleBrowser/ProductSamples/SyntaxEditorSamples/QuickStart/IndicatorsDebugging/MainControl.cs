...
public partial class MainControl : UserControl, IProductSample {
	...
	public MainControl() {
		...
		// Register built-in classification types (so breakpoint indicator styles are registered)
	...
	private void OnStartDebuggingToolStripButtonClick(object sender, EventArgs e) {
		...
		stopDebuggingToolStripButton.Enabled = _currentStatementSnapshotOffset.HasValue;
	...
	private void ShowElapsedTime(ITextSnapshotLine snapshotLine, TimeSpan timeSpan) {
		...
		if (editor.Document.Properties.TryGetValue<ElapsedTimeTagger>(out var tagger)) {
			...
			var tag = new ElapsedTimeTag(timeSpan) {
				Size = new Size(editor.ActiveView.DefaultCharacterWidth, height: 0)
			};
	...
	protected override void OnHandleCreated(EventArgs e) {
		...
		BeginInvoke(() => {
			...
			DebuggingHelper.ToggleBreakpoint(new TextSnapshotOffset(editor.ActiveView.CurrentSnapshot, editor.ActiveView.CurrentSnapshot.Lines[19].StartOffset), isEnabled: true);
			DebuggingHelper.ToggleBreakpoint(new TextSnapshotOffset(editor.ActiveView.CurrentSnapshot, editor.ActiveView.CurrentSnapshot.Lines[23].StartOffset), isEnabled: false);
			DebuggingHelper.ToggleBreakpoint(new TextSnapshotOffset(editor.ActiveView.CurrentSnapshot, editor.ActiveView.CurrentSnapshot.Lines[28].StartOffset), isEnabled: true);
		});
	}
