...
public partial class MainControl : UserControl {
	...
	public MainControl() {
		...
		// Configure each editor with the custom syntax language that is configured to support file differences
		var syntaxLanguage = new CustomSyntaxLanguage();
		oldEditor.Document.Language = syntaxLanguage;
		newEditor.Document.Language = syntaxLanguage;
	...
	private TextViewScrollState CreateScrollStateForTopLineDifferenceIndex(IEditorView view, int topLineDifferenceIndex) {
		...
		// Get the differences associated with the view, quitting if differences have yet to be defined
		if (GetViewDifferences(view) is not { } differences)
			return scrollState;
		...
		#else
		var diffList = new List<IDifference>(differences);
		#endif
		...
		if (hasTopImaginaryLines) {
			...
			if (topLineDifferenceIndex <= firstRealDifferenceIndex) {
				...
				scrollState.VerticalAnchorTextPosition = new TextPosition(line: 0, character: 0);
		...
		if (!isImaginary) {
			...
			scrollState.VerticalAnchorTextPosition = new TextPosition(line: differenceLine.Position!.Value, character: 0);
		}
		else {
			...
			if (previousRealDifferenceLineIndex >= 0) {
				...
				scrollState.VerticalAnchorTextPosition = new TextPosition(line: realDifferenceLine.Position!.Value, character: 0);
	...
	private int GetViewTopLineDifferenceIndex(IEditorView view) {
		...
		var offsetLineCount = (int)Math.Floor((double)scrollState.VerticalAmountOffset / view.DefaultLineHeight);
	...
	/// <summary>
	/// Occurs then the CheckBox.Checked property changes.
	/// </summary>
	/// <param name="sender">The sender of the event.</param>
	/// <param name="e">The event args.</param>
	private void OnIgnoreWhiteSpaceCheckBoxCheckedChanged(object sender, EventArgs e) {
