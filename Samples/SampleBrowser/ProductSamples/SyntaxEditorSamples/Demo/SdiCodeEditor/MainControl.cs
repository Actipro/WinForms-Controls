...
public partial class MainControl : UserControl, IProductSample {
	...
	private readonly int _defaultIndicatorMarginWidth;
	private bool _hasPendingParseData;

	// Project assemblies used by C#/VB in the .NET Languages Add-on
	private readonly ActiproSoftware.Text.Languages.CSharp.Implementation.CSharpProjectAssembly _cSharpProjectAssembly;
	private readonly ActiproSoftware.Text.Languages.VB.Implementation.VBProjectAssembly _vbProjectAssembly;
	...
	public MainControl() {
		...
		_defaultFontSize = editor.Font.Size;
		_defaultIndicatorMarginWidth = editor.IndicatorMarginWidth;
		...
		_cSharpProjectAssembly = new ActiproSoftware.Text.Languages.CSharp.Implementation.CSharpProjectAssembly("SampleBrowser");
		_vbProjectAssembly = new ActiproSoftware.Text.Languages.VB.Implementation.VBProjectAssembly("SampleBrowser");
		...
		// Register built-in classification types
	...
	private void LoadLanguage(ISyntaxLanguage language) {
		...
		symbolSelector.Visible = (language.GetNavigableSymbolProvider() is not null);
	...
	private void LoadLanguage(string languageKey) {
		...
		switch (languageKey) {
			...
			case "C# (in .NET Languages Add-on)": {
				...
				language.RegisterService<ActiproSoftware.Text.Languages.DotNet.Reflection.IProjectAssembly>(_cSharpProjectAssembly);
			...
			case "CSS":
				...
				break;
			case "Custom...":
				BeginInvoke(() => { LoadLanguageDefinitionFromFile(filename: null); });
				break;
			case "HTML":
			...
			case "JavaScript (in Web Languages Add-on)":
				...
				LoadLanguage(new ActiproSoftware.Text.Languages.JavaScript.Implementation.JavaScriptSyntaxLanguage());
				break;
			case "JSON (in Web Languages Add-on)":
				// Web Languages Add-on JSON language
				LoadLanguage(new ActiproSoftware.Text.Languages.JavaScript.Implementation.JsonSyntaxLanguage());
				break;
			case "JSON with Comments (in Web Languages Add-on)":
				// Web Languages Add-on JSON language
				LoadLanguage(new ActiproSoftware.Text.Languages.JavaScript.Implementation.JsonSyntaxLanguage(areCommentsSupported: true));
				break;
			case "Lua":
			...
			case "Python (in Python Language Add-on)":
				...
				LoadLanguage(new ActiproSoftware.Text.Languages.Python.Implementation.PythonSyntaxLanguage());
			...
			case "VB (in .NET Languages Add-on)": {
				...
				language.RegisterService<ActiproSoftware.Text.Languages.DotNet.Reflection.IProjectAssembly>(_vbProjectAssembly);
				LoadLanguage(language);
			...
			case "XML (in Web Languages Add-on)":
				...
				LoadLanguage(new ActiproSoftware.Text.Languages.Xml.Implementation.XmlSyntaxLanguage());
				break;
			default:
				// Plain text
	...
	/// <summary>
	/// Logs a message to the events <see cref="ListBox"/>.
	/// </summary>
	...
	private void OnFontSizeToolStripMenuItemDropDownItemClicked(object sender, ToolStripItemClickedEventArgs e) {
		if (int.TryParse(e.ClickedItem?.Text, out var fontSize)) {
			editor.Font = new Font(editor.Font.FontFamily, fontSize);

			// Scale the indicator margin width based on the size of the font
			var fontScale = editor.Font.Size / _defaultFontSize;
			editor.IndicatorMarginWidth = (int)Math.Round(_defaultIndicatorMarginWidth * fontScale, MidpointRounding.AwayFromZero);
		}
	}
	...
	private void OnLanguageToolStripMenuItemDropDownItemClicked(object sender, ToolStripItemClickedEventArgs e) {
		if ((e.ClickedItem is ToolStripMenuItem) && (e.ClickedItem.Enabled) && (e.ClickedItem.Text is not null))
			LoadLanguage(e.ClickedItem.Text);
	}
		...
		try {
			switch (e.ClickedItem?.Name) {
				...
				case nameof(exitToolStripMenuItem):
					BeginInvoke(() => { MessageBox.Show("Close the application here."); });
					break;
				...
				case nameof(importVisualStudioSettingsToolStripMenuItem):
					BeginInvoke(OpenVSSettingsFile);
					break;
				case nameof(insertLorumIpsumToolStripMenuItem):
					editor.ActiveView.SelectedText = new ActiproSoftware.Text.Utility.LipsumGenerator().GenerateParagraph(useStartingWords: true, wordCount: 30);
					break;
				...
				case nameof(newToolStripMenuItem):
					NewFile();
					break;
				case nameof(normalizeLineTerminatorsToCRLFToolStripMenuItem):
					editor.ActiveView.TextChangeActions.NormalizeLineTerminatorsToCRLF();
					break;
				case nameof(normalizeLineTerminatorsToLFToolStripMenuItem):
					editor.ActiveView.TextChangeActions.NormalizeLineTerminatorsToLF();
					break;
				case nameof(openDocumentToolStripButton):
				case nameof(openToolStripMenuItem):
					BeginInvoke(() => OpenFile());
					break;
				...
				case nameof(saveToolStripMenuItem):
					...
					BeginInvoke(() => { MessageBox.Show("Save the document here."); });
	...
	private void OnSyntaxEditorDocumentChanged(object sender, EditorDocumentChangedEventArgs e) {
		if (!editor.IsDisposed)
			LogEvent(nameof(SyntaxEditor.DocumentChanged));
	}
	...
	private void OnSyntaxEditorDocumentIsModifiedChanged(object sender, EventArgs e)
		=> LogEvent(string.Format("{0}: IsModified={1}", nameof(SyntaxEditor.DocumentIsModifiedChanged), editor.Document.IsModified));
	...
	private void OnSyntaxEditorIsOverwriteModeActiveChanged(object sender, EventArgs e) {
		LogEvent(nameof(SyntaxEditor.IsOverwriteModeActiveChanged));
	...
	private void OnSyntaxEditorMacroRecordingStateChanged(object sender, EventArgs e) {
		LogEvent($"{nameof(SyntaxEditor.MacroRecordingStateChanged)}: {editor.MacroRecording.State}");
	...
	private void OnSyntaxEditorUserInterfaceUpdate(object sender, EventArgs e) {
		...
		if (_hasPendingParseData) {
			...
			if (editor.Document.ParseData is ILLParseData parseData) {
	...
	private void OnSyntaxEditorViewIsIncrementalSearchActiveChanged(object sender, TextViewEventArgs e) {
		if ((e.View is IEditorView editorView) && (!editorView.IsIncrementalSearchActive)) {
	...
	private void OnSyntaxEditorViewSearch(object sender, EditorViewSearchEventArgs e) {
		...
		if (e.ResultSet.OperationType == SearchOperationType.FindNextIncremental) {
			...
			var notFoundMessage = (notFound ? " (not found)" : string.Empty);
	...
	private void OnSyntaxEditorViewSplitAdded(object sender, EventArgs e)
		=> LogEvent(nameof(SyntaxEditor.ViewSplitAdded));
	...
	private void OnSyntaxEditorViewSplitMoved(object sender, EventArgs e)
		=> LogEvent(nameof(SyntaxEditor.ViewSplitMoved));
	...
	private void OnSyntaxEditorViewSplitRemoved(object sender, EventArgs e)
		=> LogEvent(nameof(SyntaxEditor.ViewSplitRemoved));
